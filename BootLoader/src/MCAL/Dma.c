...
#define CLR_STREAMx_FLAGS      (0x3D)

/*  THIS SEQUENCE OF REGESTER IS REPEATED FOR THE 8 STREAM */
typedef struct
{
	volatile u32 DMA_SxCR;       // CONTROL REGESTER
	volatile u32 DMA_SxNDTR;     //SET THE COUNTER OF DATA TO BE TRASLATE
	volatile u32 DMA_SxPAR;      // THE ADDRESS OF THE PERPHERAL
	volatile u32 DMA_SxM0AR;     // SET THE ADDRESS OF MEMORY
	volatile u32 DMA_SxM1AR;     // SET THE SECOUND ADDRESS OF THE MEMORY IN CASE OF DOUBLE BUFFER MODE
...
typedef struct
{
	volatile u32 DMA_LISR;      // READ THE INTERRUPT FLAGS FOR STREAM FROM 0 TO 3
	volatile u32 DMA_HISR;      // READ THE INTERRUPT FLAGS FOR STREAM FROM 4 TO 7
	volatile u32 DMA_LIFCR;     // CLEAR THE CORRESPONDING INTERRUPT FLAG IN ISR REGIESTER BY WRITE 1 FOR STREAM FROM 0 TO 3
	volatile u32 DMA_HIFCR;     // CLEAR THE CORRESPONDING INTERRUPT FLAG IN ISR REGIESTER BY WRITE 1 FOR STREAM FROM 4 TO 7
	volatile StreamReg Streams[8];
}DAM_reg_t;
...
Dma_ErrorState_t DMA_Init(Dma_cfg_t* DmaCfg)
{
	...
	{
		Loc_DMA_Reg->Streams[DmaCfg->Stream].DMA_SxPAR = DmaCfg->DisAdd;
		Loc_DMA_Reg->Streams[DmaCfg->Stream].DMA_SxM0AR =DmaCfg->SrcAdd;
	}
	/* DATA DIRECTION */
...
{
	DAM_reg_t  *Loc_DMA_Reg = DmaCfg->DMA_Num;
	/* CLEAR ALL THE FLAG BEFORE START */
	//	if(DmaCfg->Stream < 4)
	//	{
	//		if(DmaCfg->Stream > 1)
	//		{
	//			Loc_DMA_Reg->DMA_LIFCR |= (CLR_STREAMx_FLAGS << (DmaCfg->Stream*6));
	//		}
	//		else
	//		{
	//			Loc_DMA_Reg->DMA_LIFCR |= (CLR_STREAMx_FLAGS << (((DmaCfg->Stream-2)*6+16)));
	//		}
	//	}
	//	else
	//	{
	//		if(DmaCfg->Stream < 6)
	//		{
	//			Loc_DMA_Reg->DMA_HIFCR |= (CLR_STREAMx_FLAGS << (DmaCfg->Stream-4)*6);
	//		}
	//		else
	//		{
	//			Loc_DMA_Reg->DMA_HIFCR |= (CLR_STREAMx_FLAGS << (((DmaCfg->Stream-6)*6)+16));
	//		}
	//	}
